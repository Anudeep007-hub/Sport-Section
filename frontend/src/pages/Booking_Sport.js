...
const BookSlot = ({ sportId }) => {
    ...
    useEffect(() => {
        const fetchBookedSlots = async () => {
            try {
                const response = await fetch(`http://localhost:4000/sport/booking/`);
                if (!response.ok) {
        ...
        const fetchAvailableSlots = async () => {
            try {
                const response = await fetch(`http://localhost:4000/sport/slots/${sportId}`);
                if (!response.ok) {
                    throw new Error(`HTTP error! status: ${response.status}`);
                }
                const data = await response.json();
    ...
    const handleBooking = () => {
        ...
        fetch("http://localhost:4000/sport/booking/create", {
            method: "POST",
            headers: {
